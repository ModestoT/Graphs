...
def bft(room, graph):
    ...
    while len(queue) > 0:
        ...
        if last_room not in visited:
            ...
            # if one of the directions has not been traveled to return the path
            for direction in graph[last_room]:
                if graph[last_room][direction] == '?':
                    return path
            # else enqueue each of its neighbors in the queue
            for i in graph[last_room]:
                direction = graph[last_room][i] 
